...
# Title: Meditation Visuals (Minute-Splits)
...
for (d in 1:length(med_fn_list[["day"]])) {
  ...
  # Set frame counter for each min in a 5-min recording ---------------------
  for (min in 1:5) {
    # Minute 1
    if (min == 1) {
      b = 1
    ...
    while (b < (length(med_data_list[["Meditation"]]) / 5) * min + 1) {
      if (b > 0 & b < 10) {
        png(
          file = paste0("med_rec_00", b, "_", tools::file_path_sans_ext(basename(med_data_file)), ".png", sep = ""),
          width = 1.5,
          height = 1.5,
          units = "in",
          res = 600
      ...
      if (b == 10) {
        png(
          file = paste0("med_rec_0", b, "_", tools::file_path_sans_ext(basename(med_data_file)), ".png", sep = ""),
          width = 1.5,
          height = 1.5,
          units = "in",
          res = 600
      ...
      if (b == 100) {
        png(
          file = paste0("med_rec_", b, "_", tools::file_path_sans_ext(basename(med_data_file)), ".png", sep = ""),
          width = 1.5,
          height = 1.5,
          units = "in",
          res = 600
